...
const BookDetails = ({ book: { isbn }, onShowList }: Props): ReactElement => {
  const path = `/book/${isbn}`
  const [book] = useBookApi<Book>("get", path);
  const deleteBook = () => { bookApi<string>("delete", path, onShowList) };
  ...
  return (
    <>
      ...
      <div className="ui divider"></div>
      <button className="ui blue button" onClick={onShowList}>
        Back
      </button>
    </>
  )
